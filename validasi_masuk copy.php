...
if (mysqli_num_rows($result) > 0) {
    ...
    // Check if there is an existing record with the same nik and date
    $checkQuery = "SELECT COUNT(*) as count FROM user_log WHERE nik = '$no_id' AND date = '$date'";
    ...
    if ($count > 0) {
        ...
        $insertQuery = "INSERT INTO user_log (nik, date, time_stamp_in, time_stamp_out, calculation_time, `condition`)
                        VALUES ('$no_id', '$date', '$time_stamp_in', '$time_formatted', '$calculation_time', '$condition_4')";
                        echo "<div style='text-align: center; color: red;'><h5 class='card-title'>KARTU INI SUDAH DISCAN</h5></div>";
    } else {
        // If no record exists, insert a new entry with condition_1 = "3"
        $insertQuery = "INSERT INTO user_log (nik, date, time_stamp_in, time_stamp_out, calculation_time, `condition`)
                        VALUES ('$no_id', '$date', '$time_stamp_in', '$time_formatted', '$calculation_time', '$condition_3')";
    }
...
$mysqli->close();
    ...
    Quagga.init({
        ...
        decoder: {
            readers: ["ean_reader", "upc_reader"], // Format barcode yang akan di-scan
        },
    });

    // Tangani hasil pemindaian barcode
    Quagga.onDetected(function(result) {
        var code = result.codeResult.code;
        document.getElementById("no_id").value = code;
    });
